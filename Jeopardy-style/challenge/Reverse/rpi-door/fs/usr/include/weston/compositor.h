...
struct weston_renderer {
	...
	void (*surface_set_brightness)(struct weston_surface *surface,
			       float brightness);
...
struct weston_binding;
typedef void (*weston_modifier_binding_handler_t)(struct wl_seat *seat,
					          enum weston_keyboard_modifier modifier,
					          void *data);
struct weston_binding *
...
struct weston_surface_animation *
weston_dim_run(struct weston_surface *surface,
		float start, float end, uint32_t duration,
		weston_surface_animation_done_func_t done, void *data);
...
void
weston_surface_set_brightness(struct weston_surface *surface,
			 float brightness);
